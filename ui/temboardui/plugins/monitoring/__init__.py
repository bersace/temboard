            ...
            try:
                with conn.begin(), stopwatch:
                    res = conn.execute(
                        ...
                        (config["name"], config["record_type"], config["aggregate"]),
                    )
            ...
            try:
                with conn.begin(), stopwatch:
                    res = conn.execute(
                        ...
                        (config["name"], config["record_type"], config["history"]),
                    )
                    # Call here pg_sleep() using conn.execute() to fake slow
                    # archiving.
                    table_name, nb_rows = res.fetchone()
...
def check_data_worker(app, host_id, instance_id, data):
    ...
    check_preprocessed_data(app, worker_session, host_id, instance_id)
    ...
    with engine.connect() as conn:
        ...
        for tablename in tablenames:
            ...
            if tablename.endswith("_history"):
                ...
                query = purge_query_base + "datetime < (NOW() - ':nday days'::INTERVAL)"
...
def notify_state_change(app, check_id, key, value, state, prev_state):
    ...
    check = (
        worker_session.query(Check)
        .filter(Check.check_id == check_id)
        .join(Instance)
        .join(Host)
        .one()
    )
    ...
    subject = "[temBoard] {state} {hostname} - {description}".format(
        hostname=hostname, state=state, description=description
    )
...
def schedule_collector(app):
    ...
    with engine.connect() as conn:
        ...
        for batch in grouper(16, res.fetchall()):
            ...
            logger.info("Scheduling batch collector for %s agents.", len(batch))
...
def collector(app, address, port, engine=None):
    ...
    try:
        ...
        collector_status = (
            worker_session.query(CollectorStatus)
            .filter(CollectorStatus.instance_id == instance_id)
            .first()
        )
    ...
    if discover_etag:
        if discover_etag != instance.discover_etag:
            ...
            logger.debug(
                "New etag. old=%s new=%s", instance.discover_etag, discover_etag
            )
